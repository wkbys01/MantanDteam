        ...
        <ul class="main_category_bar flex_bar">
          @foreach($allMainCategories as $mc)
            <li class="main_category {{ $mainCategory->id === $mc->id ? 'active' : '' }}">
              <a href="{{ route('menu_page', ['mainCategoryId' => $mc->id]) }}">
        ...
        <ul class="sub_category_bar flex_bar">
          @foreach($mainCategory->subCategories as $sc)
            <li class="sub_category {{ $subCategory->id === $sc->id ? 'active' : '' }}">
              <a href="{{ route('menu_page.sub', ['mainCategoryId' => $mainCategory->id, 'subCategoryId' => $sc->id]) }}">
          ...
          <ul class="menu_list">
            ...
            <li class="menu_content">
              ...
              >
                ...
                <div class="menu_txt">
                  <div class="menu_name"><p>{{ $menu->translations['ja'] ?? '商品名' }}</p></div>
                  <div class="price_txt"><p>{{ $menu->price }}円 (税込)</p></div>
                </div>
              </button>
            </li>
            @endforeach
